...
(function () {
    ...
    /* ==========================================================
       3. AUTOPLAY VIDEOS — plays regardless of the
       prefers-reduced-motion setting, since these are actual page
       content (background + showcase clip), not decorative
       animation. Retries play() (the bare autoplay attribute alone
       can silently fail) and nudges it awake again if a mobile OS
       pauses it while the tab/app is backgrounded. Every video here
       has a poster image, so it never shows a stalled black frame.

       Some mobile setups (iOS Low Power Mode is the common one)
       block autoplay entirely at the OS level regardless of muted/
       playsinline — no JS can override that pre-emptively. The only
       thing that reliably works there is a real user gesture, so as
       a last resort every video that fails to autoplay is retried on
    ...
    function armGestureRetry() {
        ...
        document.addEventListener('touchstart', retryAll, { once: true, passive: true });
    ...
    function ensureVideoPlays(video) {
        ...
        function tryPlay() {
            video.muted = true; // some mobile autoplay checks look at the JS property, not just the attribute
            video.play().catch(() => {
                if (!stalledVideos.includes(video)) stalledVideos.push(video);
                armGestureRetry();
            });
        }
        tryPlay();
    ...
    function initHeroVideo() {
        ...
        // Only treat a genuinely slow connection (2G/slow-2G) as constrained.
        // `saveData` alone used to be enough to skip autoplay entirely, but
        // that flag is also true on Windows whenever the user has "metered
        // connection" toggled on network settings — common even on fast
        // broadband/Wi-Fi (people leave it on to cap OneDrive/Update usage)
        // — which was silently and permanently disabling the background
        // video's autoplay with no way to recover it.
        const conn = navigator.connection || navigator.webkitConnection || navigator.mozConnection;
        const isSlow = !!(conn && /2g/.test(conn.effectiveType || ''));

        if (isSlow) {
            video.removeAttribute('autoplay');
            video.preload = 'none';
            // poster stays the visible fallback, but a first tap/click still
            // starts it — never fully give up on playing the video.
            if (!stalledVideos.includes(video)) stalledVideos.push(video);
            armGestureRetry();
            return;
        }
    ...
    function initShowcaseVideo() {
        ensureVideoPlays(document.getElementById('showcaseVideo'));
    }
